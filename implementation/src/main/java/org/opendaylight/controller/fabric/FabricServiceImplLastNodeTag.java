...
public class FabricServiceImplLastNodeTag implements FabricService {
    private final Logger logger = LoggerFactory.getLogger(FabricServiceImplLastNodeTag.class);
    ...
    private static int gFabricNodeNum = 0;
    //////////
     ...
     */
    public FabricServiceImplLastNodeTag(DataBroker dataBroker,
...
//        }
    ...
    private void initFabricFlows(){
        ...
        try{
            WatchFabricInstall watch = new WatchFabricInstall();
            watch.start();
            watch.join();
        }catch(Exception ex){
            logger.info("WatchFabricInstall join error!");
        }
        return;
    }
...
//        }
    ...
    private class WatchFabricInstall extends Thread{
        @Override
        public void run() {
            while(gFabricNodeNum != 0){
                try{
                Thread.sleep(100);
                }catch(InterruptedException ex){
                    logger.info("WatchFabricInstall exception!");
                }
            }
            logger.info("Fabric install over!");
            return;
        }
    }
    @Override
