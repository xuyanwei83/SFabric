 ...
 * target or not.
 */
public class PacketHandlerLastNodeTag implements PacketProcessingListener {

    private final Logger _logger = LoggerFactory.getLogger(PacketHandlerLastNodeTag.class);
    ...
    public PacketHandlerLastNodeTag(DataBroker dataService,
    ...
    private void handleIPPacket(Packet packet,IPv4 ipv4, NodeConnectorRef ingress){
        ...
        if(senderImpl == null){
            ...
            if(!this.addHostFlowQueue.contains(senderImpl)){
                try{
                    this.addHost(srcMac,BitBufferHelper.toByteArray(ipv4.getSourceAddress()),ingress);
        ...
        if(targetImpl != null){
            ...
            NodeConnectorRef ncr = this.getFabricService().getConnectorService().getNodeControllerNodeConnectorByNodeId(egress);
    ...
    private void handleArpRequestPacket(Packet packet,ARP arp, NodeConnectorRef ingress){
        ...
        if(targetImpl == null){
            ...
            try{
                ...
                this.sendPacketOut(ethernet.serialize(), this.getFabricService().getConnectorService().getNodeControllerNodeConnectorByNodeId(ingress), ingress);
                FabricImpl fabricImpl = new FabricImpl(senderImpl, targetImpl);
                if(!this.addFabricFlowQueue.contains(fabricImpl)){
                    try{
                        this.addFabricFlowQueue.put(fabricImpl);
    ...
    */
    private void floodExternalPorts(byte[] payload, NodeConnectorRef ingress) {
        ...
        HashSet<NodeConnectorRef> externalPorts = this.getFabricService().getConnectorService().getExternalNodeConnectors();
        externalPorts.remove(ingress);

        for (NodeConnectorRef egress : externalPorts) {
            this.sendPacketOut(payload, this.getFabricService().getConnectorService().getNodeControllerNodeConnectorByNodeId(egress), egress);
        }
    }
...
//        return srcFabricMap.getId();
    ...
    private boolean checkNode(NodeConnectorRef n1,NodeConnectorRef n2){
        ...
        boolean ret = groupService.cheakInnerSwitchsSameGroup(nodeId1.getValue(), nodeId2.getValue());
        return ret;
    }
    private void addHost(byte[] mac,byte[] ip,NodeConnectorRef ingress){
        try{
            this.getIfHostListener();
            InetAddress iaIp = InetAddress.getByAddress(ip);
            NodeConnector nc = NodeMapping.toADNodeConnector(ingress);
            this.hostListener.addStaticHost(iaIp.getHostAddress(),HexEncode.bytesToHexStringFormat(mac), nc, null);
        }catch(Exception ex){
        }
    }
    ...
    private synchronized void restartThread(){
        if(this.addHostFlowThread.getState() == Thread.State.TERMINATED){
            this.addHostFlowThread = new Thread(new AddHostFlow());
            this.addHostFlowThread.start();
        }
        if(this.addFabricFlowThread.getState() == Thread.State.TERMINATED){
            this.addFabricFlowThread = new Thread(new AddFabricFlow());
            this.addFabricFlowThread.start();
        }
    }
        ...
        public void run() {
            while (true) {
                try{
                    ...
                    _logger.info("AddHostFlow InterruptedException : {}",e.getMessage());
                }
            }
        }
    }
...
//                    flowWriterService.addMacToMacFlowsHostPath(src.getMac(), src.getNodeConnector());
//                    flowWriterService.addMacToMacFlowsHostPath(dst.getMac(), dst.getNodeConnector());
                }catch(InterruptedException e){
                    _logger.info("AddFabricFlow InterruptedException : {}",e.getMessage());
                }
            }
        }
    }
